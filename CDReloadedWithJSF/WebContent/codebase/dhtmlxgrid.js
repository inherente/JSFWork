...
dhtmlxArray._master={
    _dhx_find:function(a){
        for(var b=0;b<this.length;b++)if(a==this[b])return b;return-1
    },
...
function dhtmlXGridObject(a){
    ...
    a?typeof a=="object"?(this.entBox=a,this.entBox.id="cgrid2_"+this.uid()):this.entBox=document.getElementById(a):(this.entBox=document.createElement("DIV"),this.entBox.id="cgrid2_"+this.uid());
    ...
    this.entBox.style.width=this.entBox.getAttribute("width")||(window.getComputedStyle?this.entBox.style.width||window.getComputedStyle(this.entBox,null).width:this.entBox.currentStyle?
        this.entBox.currentStyle.width:this.entBox.style.width||0)||"100%";
    ...
    this.setSkin=function(e){
        this.skin_name=
        e;
        this.entBox.className="gridbox gridbox_"+e;
        ...
        switch(e){
            case "clear":
                ...
                break;
            case "dhx_skyblue":case "dhx_web":case "glassy_blue":case "dhx_black":case "dhx_blue":case "modern":case "light":
                this._srdh=20;
                this.forceDivInHeader=!0;
                break;
            case "xp":
                this.forceDivInHeader=!0;
                this._srdh=_isIE&&document.compatMode!="BackCompat"?
                25:22;
                break;
            case "mt":
                this._srdh=_isIE&&document.compatMode!="BackCompat"?25:22;
                break;
            case "gray":
                if(_isIE&&document.compatMode!="BackCompat")this._srdh=22
        }
        if(_isIE&&this.hdr){
            var a=this.hdr.parentNode;
            a.removeChild(this.hdr);
            a.appendChild(this.hdr)
        }
        this.setSizes()
    };
    ...
    this.init=function(e){
        if(this.isTreeGrid()&&!this._h2){
            ...
            if(this._fake&&
                !this._realfake)this._fake._h2=this._h2;
            this._tgc={
                imgURL:null
            }
        }
        if(this._hstyles){
            ...
            for(var a=this.hdr.insertRow(0),b=0;b<this.hdrLabels.length;b++)a.appendChild(document.createElement("TH")),a.childNodes[b]._cellIndex=b,a.childNodes[b].style.height="0px";
            _isIE&&_isIE<8?a.style.position="absolute":a.style.height="auto";
            a=this.hdr.insertRow(_isKHTML?
                2:1);
            a._childIndexes=[];
    ...
    this.setColumnSizes=function(e){
        for(var a=0,b=[],c=0;c<this._cCount;c++)this.initCellWidth[c]=="*"&&!this._hrrar[c]?(this._awdth=
            !1,b.push(c)):(this.cellWidthType=="%"?(typeof this.cellWidthPC[c]=="undefined"&&(this.cellWidthPC[c]=this.initCellWidth[c]),this.cellWidthPX[c]=Math.floor(e*this.cellWidthPC[c]/100)||0):typeof this.cellWidthPX[c]=="undefined"&&(this.cellWidthPX[c]=this.initCellWidth[c]),this._hrrar[c]||(a+=this.cellWidthPX[c]*1));
        if(b.length){
            var d=Math.floor((e-a)/b.length);
            d<0&&(d=1);
            for(c=0;c<b.length;c++){
                var g=Math.max(this._drsclmW?this._drsclmW[b[c]]||0:0,d);
                this.cellWidthPX[b[c]]=g;
                a+=g
            }
            if(e>a){
                var l=b[b.length-
                1];
                this.cellWidthPX[l]+=e-a;
                a=e
            }
            this._setAutoResize()
        }
    ...
    this.setSizes=function(){
        if(this.hdr.rows[0]){
            ...
            if(!this.entBox.offsetWidth&&(!this.globalBox||!this.globalBox.offsetWidth))this._sizeTime=window.setTimeout(function(){
                ...
                var d=(this.entBox.cmp||this._delta_x)&&(this.skin_name||"").indexOf("dhx")==0&&!e?
                2:0,g=(this.entBox.cmp||this._delta_y)&&(this.skin_name||"").indexOf("dhx")==0&&!e?2:0,l=this.parentGrid?!1:this.objBox.scrollHeight>this.objBox.offsetHeight,j=_isFF?18:18,m=this.entBox.clientWidth-(this.skin_h_correction||0)*(e?0:1)-d,n=this.entBox.clientWidth-(this.skin_h_correction||0)-d,o=this.entBox.clientHeight-g,p=this.setColumnSizes(n-(l?j:0)-(this._correction_x||0)),r=this.parentGrid?!1:this.objBox.scrollWidth>this.objBox.offsetWidth||this.objBox.style.overflowX=="scroll",s=this.hdr.clientHeight,
                t=this.ftr?this.ftr.clientHeight:0,u=m,q=o-s-t;
                this._awdth&&this._awdth[0]&&this._awdth[1]==99999&&(r=0);
                if(this._ahgr&&(q=this._ahgrMA?this.entBox.parentNode.clientHeight-s-t:this.obj.offsetHeight+(r?j:0)+(this._correction_y||0),this._ahgrM&&(q=this._ahgrF?Math.min(this._ahgrM,q+s+t)-s-t:Math.min(this._ahgrM,q)),l&&q>=this.obj.scrollHeight+(r?j:0)))l=!1,this.setColumnSizes(n-(this._correction_x||0));
                if(this._awdth&&this._awdth[0]){
                    if(this.cellWidthType=="%")this.cellWidthType="px";
                    this._fake&&(p+=
                        this._fake.entBox.clientWidth);
                    u=Math.min(Math.max(p+(l?j:0),this._awdth[2]),this._awdth[1])+(this._correction_x||0);
                ...
                this.entBox.style.width=Math.max(0,u+(e?2:0)*a+this._ff_size_delta)+"px";
                this.entBox.style.height=q+(e?2:0)*a+s+t+"px";
                this.objBox.style.height=q+(e&&!l?2:0)*a+"px";
                this.hdrBox.style.height=s+"px";
                q!=o&&this.doOnScroll(0,!this._srnd);
                var v=
                this["setSizes_"+this.skin_name];
                v&&v.call(this);
                this.setSortImgPos();
    ...
    this.setInitWidths=function(e){
        this.cellWidthType="px";
        this.initCellWidth=e.split(this.delim);
        if(_isFF)for(var a=0;a<this.initCellWidth.length;a++)this.initCellWidth[a]!="*"&&(this.initCellWidth[a]=parseInt(this.initCellWidth[a]))
    };
    ...
    this.changeCursorState=function(a){
        ...
        if(b){
            ...
            b.style.cursor=b.offsetWidth-
            (a.offsetX||(parseInt(this.getPosition(b,this.hdrBox))-c)*-1)<(_isOpera?20:10)?"E-resize":"default";
    ...
    this.startColResize=function(a){
        ...
        var b=a.target||a.srcElement;
        b.tagName!="TD"&&(b=this.getFirstParentOfType(b,"TD"));
        var h=a.clientX,f=this.hdr.offsetWidth,d=parseInt(b.offsetWidth);
        if(b.tagName=="TD"&&b.style.cursor!="default"&&(!this._drsclmn||this._drsclmn[b._cellIndex]))c._old_d_mm=document.body.onmousemove,
            c._old_d_mu=document.body.onmouseup,document.body.onmousemove=function(a){
                c&&c.doColResize(a||window.event,b,d,h,f)
    ...
    this.doColResize=function(a,b,c,f,d){
        ...
        this.resized=b;
        var g=c+(a.clientX-f),l=d+(a.clientX-f);
        if(this.callEvent("onResize",
            [b._cellIndex,g,this])){
            if(_isIE)this.objBox.scrollLeft=this.hdrBox.scrollLeft;
            if(b.colSpan>1){
                for(var j=[],m=0;m<b.colSpan;m++)j[m]=Math.round(g*this.hdr.rows[0].childNodes[b._cellIndexS+m].offsetWidth/b.offsetWidth);
                for(m=0;m<b.colSpan;m++)this._setColumnSizeR(b._cellIndexS+m*1,j[m])
            }else this._setColumnSizeR(b._cellIndex,g);
            this.doOnScroll(0,1);
            this.setSizes();
    ...
    this._setColumnSizeR=function(a,b){
        if(b>(this._drsclmW&&!this._notresize?this._drsclmW[a]||
            10:10)){
    ...
    this.setSortImgState=function(a,b,c,f){
        ...
        if(convertStringToBoolean(a)){
            this.sortImg.src=
            c=="asc"?this.imgURL+"sort_asc.gif":this.imgURL+"sort_desc.gif";
            this.sortImg.style.display="";
            this.fldSorted=this.hdr.rows[0].childNodes[b];
            var d=this.hdr.rows[f||1];
            if(d){
                for(var g=0;g<d.childNodes.length;g++)if(d.childNodes[g]._cellIndexS==b)return this.r_fldSorted=d.childNodes[g],this.setSortImgPos();return this.setSortImgState(a,b,c,(f||1)+1)
            }
        }else this.sortImg.style.display="none",this.fldSorted=null
    };
    
    this.setSortImgPos=function(a,b,c,f){
        if(!this._hrrar||!this._hrrar[this.r_fldSorted?this.r_fldSorted._cellIndex:
            a])if(f||(f=a?this.hdr.rows[c||0].cells[a]:this.r_fldSorted),f!=null){
            var d=this.getPosition(f,this.hdrBox),g=f.offsetWidth;
            this.sortImg.style.left=Number(d[0]+g-13)+"px";
    ...
    this.setActive=function(a){
        ...
        a==!0?(globalActiveDHTMLGridObject&&globalActiveDHTMLGridObject!=
            this&&(globalActiveDHTMLGridObject.editStop(),globalActiveDHTMLGridObject.callEvent("onBlur",[globalActiveDHTMLGridObject])),globalActiveDHTMLGridObject=this,this.isActive=!0):(this.isActive=!1,this.callEvent("onBlur",[this]))
    };
    ...
    this._doContClick=function(a){
        ...
        if(!b||typeof b.parentNode.idd=="undefined")return!0;
        if(a.button==2||_isMacOS&&a.ctrlKey){
            ...
            if(this._ctmndx){
                if(!this.callEvent("onBeforeContextMenu",[b.parentNode.idd,b._cellIndex,
                    this]))return!0;
                ...
                if(this._ctmndx.showContextMenu){
                    var f=window.document.documentElement,d=window.document.body,g=[f.scrollLeft||d.scrollLeft,f.scrollTop||d.scrollTop];
                    if(_isIE)var l=a.clientX+g[0],j=a.clientY+g[1];else l=a.pageX,j=a.pageY;
                    this._ctmndx.showContextMenu(l-1,j-1);
                    this.contextID=this._ctmndx.contextMenuZoneId=b.parentNode.idd+"_"+b._cellIndex;
                    this._ctmndx._skip_hide=!0
                }else b.contextMenuId=b.parentNode.idd+"_"+
                    b._cellIndex,b.contextMenu=this._ctmndx,b.a=this._ctmndx._contextStart,b.a(b,a),b.a=null;
                a.cancelBubble=!0;
                return!1
            }
    ...
    this.doClick=function(a,b,h,f){
        this.selMultiRows||(h=0);
        var d=this.row?this.row.idd:0;
        this.setActive(!0);
        h||(h=0);
        if(this.cell!=null)this.cell.className=this.cell.className.replace(/cellselected/g,"");
        if(a.tagName=="TD"){
            if(this.checkEvent("onSelectStateChanged"))var g=
                this.getSelectedId();
            var l=this.row;
            if(h==1){
                var j=this.rowsCol._dhx_find(a.parentNode),m=this.rowsCol._dhx_find(this.lastClicked);
                if(j>m)var n=m,o=j;else n=j,o=m;
                for(var p=0;p<this.rowsCol.length;p++)if(p>=n&&p<=o)if(this.rowsCol[p]&&!this.rowsCol[p]._sRow)this.rowsCol[p].className.indexOf("rowselected")==-1&&this.callEvent("onBeforeSelect",[this.rowsCol[p].idd,d])&&(this.rowsCol[p].className+=" rowselected",this.selectedRows[this.selectedRows.length]=this.rowsCol[p]);else return this.clearSelection(),
                    this.doClick(a,b,0,f)
            }else if(h==2&&a.parentNode.className.indexOf("rowselected")!=-1){
            ...
            if(!r&&!a.parentNode._sRow)if(this.callEvent("onBeforeSelect",[a.parentNode.idd,d])){
                if(h==0){
                    if(this.getSelectedRowId()==a.parentNode.idd)return this.cell=a;
                    this.clearSelection()
                }
                this.cell=a;
            ...
            if(this.checkEvent("onSelectStateChanged")){
                ...
                g!=v&&this.callEvent("onSelectStateChanged",[v,g])
            }
        }
        this.isActive=!0;
    ...
    this.moveToVisible=function(a,b){
        ...
        try{
            if(a.offsetHeight){
                var f=a.offsetLeft+a.offsetWidth+20,d=0;
                f>this.objBox.offsetWidth+this.objBox.scrollLeft?a.offsetLeft>this.objBox.scrollLeft&&(d=a.offsetLeft-5):a.offsetLeft<this.objBox.scrollLeft&&(f-=a.offsetWidth*2/3,f<this.objBox.scrollLeft&&(d=a.offsetLeft-5));
                if(d&&!b)this.objBox.scrollLeft=
                    d
            }
            if(a.offsetHeight)f=a.offsetTop;else var g=this._realfake?this._fake.rowsAr[a.parentNode.idd]:a.parentNode,f=this.rowsBuffer._dhx_find(g)*this._srdh;
            var l=f+a.offsetHeight+38;
            if(l>this.objBox.offsetHeight+this.objBox.scrollTop)var j=l-this.objBox.offsetHeight;else f<this.objBox.scrollTop&&(j=f-5);
            if(j)this.objBox.scrollTop=j
        }catch(m){}
    };
    ...
    this.editCell=function(){
        if(!(this.editor&&this.cell==this.editor.cell)){
            ...
            if(this.editor!=null){
                ...
                this.callEvent("onEditCell",[0,this.row.idd,this.cell._cellIndex])!=!1&&this.editor.edit?(this._Opera_stop=(new Date).valueOf(),a.className+=" editable",this.editor.edit(),this.callEvent("onEditCell",[1,this.row.idd,this.cell._cellIndex])):this.editor=null
            }
        }
    };
    ...
    this.editStop=function(a){
        ...
        if(this.editor&&
            this.editor!=null)if(this.editor.cell.className=this.editor.cell.className.replace("editable",""),a){
            ...
            var c=this.editor;
            this.editor=null;
            var f=this.callEvent("onEditCell",[2,this.row.idd,this.cell._cellIndex,c.getValue(),c.val]);
            if(typeof f=="string"||typeof f=="number")c[c.setImage?"setLabel":"setValue"](f);
            else if(!f)c[c.setImage?"setLabel":"setValue"](c.val);
    ...
    this._getNextCell=function(a,b,c){
        var a=a||this.cell,f=a.parentNode;
        this._tabOrder?(c=this._tabOrder[a._cellIndex],typeof c!="undefined"&&(a=c<0?this._nextRowCell(f,b,Math.abs(c)-1):f.childNodes[c])):(c=a._cellIndex+b,c>=0&&c<
            this._cCount?(f._childIndexes&&(c=f._childIndexes[a._cellIndex]+b),a=f.childNodes[c]):a=this._nextRowCell(f,b,b==1?0:this._cCount-1));
    ...
    this.scrollPage=function(a){
        if(this.rowsBuffer.length){
            var b=this._realfake?this._fake:this,c=Math.floor((b._r_select||this.getRowIndex(this.row.idd)||0)+a*this.objBox.offsetHeight/(this._srdh||20));
            c<0&&(c=0);
            ...
            if(this._srnd&&!this.rowsBuffer[c]){
                ...
                if(this._fake)this._fake.objBox.scrollTop=
    ...
    this.doKey=function(a){
        ...
        if(globalActiveDHTMLGridObject&&
            this!=globalActiveDHTMLGridObject)return globalActiveDHTMLGridObject.doKey(a);
    ...
    this.deleteSelectedRows=function(){
        ...
        if(a!=0){
            ...
            for(var c=a-1;c>=0;c--){
                var f=b[c];
                if(this.deleteRow(f.idd,f)){
                    if(f==this.row)var d=c
                }else this.selectedRows[this.selectedRows.length]=f
            }
            if(d)try{
                d+1>this.rowsCol.length&&d--,this.selectCell(d,0,!0)
            }catch(g){
    ...
    this.getSelectedRowId=function(){
        for(var a=[],b={},c=0;c<this.selectedRows.length;c++){
            var f=
            this.selectedRows[c].idd;
            b[f]||(a[a.length]=f,b[f]=!0)
        }
    ...
    this.setColWidth=function(a,b){
        this._hrrar[a]||(this.cellWidthType=="px"?this.cellWidthPX[a]=parseInt(b):this.cellWidthPC[a]=parseInt(b),this.setSizes())
    };
    
    this.getRowIndex=function(a){
        for(var b=0;b<this.rowsBuffer.length;b++)if(this.rowsBuffer[b]&&this.rowsBuffer[b].idd==
            a)return b;return-1
    };
    ...
    this.changeRowId=function(a,b){
        if(a!=b){
            ...
            this.rowsAr[a]=null;
            this.rowsAr[b]=c;
            for(var f=0;f<c.childNodes.length;f++)if(c.childNodes[f]._code)c.childNodes[f]._code=
                this._compileSCL(c.childNodes[f]._val,c.childNodes[f]);if(this._mat_links&&this._mat_links[a]){
                var d=this._mat_links[a];
                delete this._mat_links[a];
                for(var g in d)for(f=0;f<d[g].length;f++)this._compileSCL(d[g][f].original,d[g][f])
            }
    ...
    this.getColLabel=this.getColumnLabel=function(a,b,c){
        for(var f=(c||this.hdr).rows[(b||0)+1],d=0;d<f.cells.length;d++)if(f.cells[d]._cellIndexS==a)return _isIE?f.cells[d].innerText:f.cells[d].textContent;return""
    };
    ...
    this.setRowTextStyle=function(a,b){
        ...
        if(c)for(var d=0;d<c.childNodes.length;d++){
            ...
            c.childNodes[d].style.cssText=k+"width:"+c.childNodes[d].style.width+";"+b
        }
    };
    ...
    this.setCellTextStyle=function(a,b,c){
        ...
        if(d){
            var k=d.childNodes[d._childIndexes?d._childIndexes[b]:b];
            if(k){
                var g="";
                k.style.cssText=g+"width:"+k.style.width+";"+c
            }
        }
    };
    ...
    this.clearSelection=function(){
        this.editStop();
        for(var a=
            0;a<this.selectedRows.length;a++){
    ...
    this.copyRowContent=function(a,b){
        var c=this.getRowById(a);
        if(this.isTreeGrid())this._copyTreeGridRowContent(c,a,b);else for(var d=0;d<c.cells.length;d++)this.cells(b,d).setValue(this.cells(a,
            d).getValue());
    ...
    this.setColumnLabel=function(a,b,c,d){
        var k=(d||this.hdr).rows[c||1],g=k._childIndexes?k._childIndexes[a]:a;
        if(k.cells[g]){
            if(this.useImagesInHeader){
                k.cells[g].style.textAlign="left";
                k.cells[g].innerHTML="<img src='"+this.imgURL+""+b+"' onerror='this.src = \""+this.imgURL+"imageloaderror.gif\"'>";
                var l=new Image;
                l.src=this.imgURL+""+
                b.replace(/(\.[a-z]+)/,".des$1");
                this.preloadImagesAr[this.preloadImagesAr.length]=l;
                var j=new Image;
                j.src=this.imgURL+""+b.replace(/(\.[a-z]+)/,".asc$1");
                this.preloadImagesAr[this.preloadImagesAr.length]=j
            }else{
                ...
                m+=b;
                m+="</div>";
                k.cells[g].innerHTML=m;
                if(this._hstyles[g])k.cells[g].style.cssText=
                    this._hstyles[g]
            }
            if((b||"").indexOf("#")!=-1){
                var o=b.match(/(^|{)#([^}]+)(}|$)/);
                if(o){
                    ...
                    if(this[p])this[p](this.forceDivInHeader?k.cells[g].firstChild:k.cells[g],g,b.split(o[0]))
                }
            }
        }
    };
    ...
    this.clearAll=function(a){
        if(this.obj.rows[0]){
            ...
            if(a){
                ...
                this._aHead=this.ftr=this.cellWidth=
                this._aFoot=null;
                this.cellType=dhtmlxArray();
                this._hrrar=[];
                this.columnIds=[];
    ...
    this.sortField=function(a,b,c){
        ...
        if(d&&d.tagName=="TH"&&this.fldSort.length-1>=d._cellIndex&&this.fldSort[d._cellIndex]!="na"){
            var k=this.getSortingState(),g=k[0]==a&&k[1]=="asc"?"des":"asc";
            if(this.callEvent("onBeforeSorting",[a,this.fldSort[a],g])){
                this.sortImg.src=this.imgURL+"sort_"+(g=="asc"?"asc":"desc")+".gif";
                if(this.useImagesInHeader){
                    var l=this.hdr.rows[1].cells[d._cellIndex].firstChild;
                    if(this.fldSorted!=null){
                        var j=this.hdr.rows[1].cells[this.fldSorted._cellIndex].firstChild;
                        j.src=j.src.replace(/(\.asc\.)|(\.des\.)/,".")
                    }
                    l.src=l.src.replace(/(\.[a-z]+)$/,"."+g+"$1")
                }
                this.sortRows(d._cellIndex,this.fldSort[d._cellIndex],g);
                this.fldSorted=d;
                this.r_fldSorted=c;
    ...
    this.setHeader=function(a,b,
        c){
        for(var d=typeof a!="object"?this._eSplit(a):[].concat(a),k=[],g=new dhtmlxArray(0),l=[],j=[],m=[],n=0;n<d.length;n++)k[k.length]=Math.round(100/d.length),g[g.length]="ed",l[l.length]="left",j[j.length]="middle",m[m.length]="na";
        this.splitSign=b||"#cspan";
        this.hdrLabels=d;
        this.cellWidth=k;
        this.initCellWidth.length||this.setInitWidthsP(k.join(this.delim),!0);
        this.cellType=g;
        this.cellAlign=l;
        this.cellVAlign=j;
        this.fldSort=m;
    ...
    this.setStyle=function(a,b,c,d){
        ...
        for(var k=["#"+this.entBox.id+" table.hdr td","#"+this.entBox.id+" table.obj td","#"+this.entBox.id+" table.obj tr.rowselected td.cellselected",
            "#"+this.entBox.id+" table.obj td.cellselected","#"+this.entBox.id+" table.obj tr.rowselected td"],g=0;!_isIE;){
            try{
                var l=document.styleSheets[g].cssRules.length
            }catch(j){
                g++;
                continue
            }
        ...
        for(var m=0;m<k.length;m++)this.ssModifier[m]&&(_isIE?document.styleSheets[0].addRule(k[m],this.ssModifier[m]):document.styleSheets[g].insertRule(k[m]+(" { "+this.ssModifier[m]+" }"),document.styleSheets[g].cssRules.length))
    };
    ...
    this._fixAlterCss=function(a){
        ...
        if(this._cssEven||this._cssUnEven)for(var b=a=a||0,c=a;c<this.rowsCol.length;c++)if(this.rowsCol[c]&&this.rowsCol[c].style.display!="none")this.rowsCol[c]._cntr?b=1:(this.rowsCol[c].className=this.rowsCol[c].className.indexOf("rowselected")!=
            -1?b%2==1?this._cssUnEven+" rowselected "+(this.rowsCol[c]._css||""):this._cssEven+" rowselected "+(this.rowsCol[c]._css||""):b%2==1?this._cssUnEven+" "+(this.rowsCol[c]._css||""):this._cssEven+" "+(this.rowsCol[c]._css||""),b++)
    };
    ...
    this.getPosition=function(a,b){
        ...
        for(var b=b||document.body,d=a,k=0,g=0;d&&d!=b;)k+=d.offsetLeft-d.scrollLeft,g+=d.offsetTop-d.scrollTop,d=d.offsetParent;
        b==document.body&&(_isIE?(g+=document.body.offsetTop||document.documentElement.offsetTop,
            k+=document.body.offsetLeft||document.documentElement.offsetLeft):_isFF||(k+=document.body.offsetLeft,g+=document.body.offsetTop));
        return[k,g]
    };
    ...
    this.obj.onclick=function(a){
        ...
        this.grid._sclE&&this.grid.editCell(a||window.event);
        (a||event).cancelBubble=!0
    };
    ...
    if(!document.body._dhtmlxgrid_onkeydown)dhtmlxEvent(document,_isOpera?"keypress":"keydown",function(a){
    ...
    this.entBox.onbeforedeactivate=function(){
        this.grid._still_active?
        this.grid._still_active=null:this.grid.isActive=!1;
        event.cancelBubble=!0
    };
...
dhtmlXGridObject.prototype={
    ...
    destructor:function(){
        ...
        for(a=0;a<this.rowsCol.length;a++)if(this.rowsCol[a])this.rowsCol[a].grid=null;for(a in this.rowsAr)this.rowsAr[a]&&(this.rowsAr[a]=null);this.rowsCol=new dhtmlxArray;
        this.rowsAr={};
    ...
    _launchCommands:function(a){
        for(var b=0;b<a.length;b++){
            for(var c=[],d=0;d<a[b].childNodes.length;d++)if(a[b].childNodes[d].nodeType==
                1)c[c.length]=a[b].childNodes[d].firstChild.data;this[a[b].getAttribute("command")].apply(this,c)
        }
    },
    _parseHead:function(a){
        var b=this.xmlLoader.doXPath("./head",a);
        if(b.length){
            var c=this.xmlLoader.doXPath("./column",b[0]),d=this.xmlLoader.doXPath("./settings",b[0]),e="setInitWidths",i=!1;
            if(d[0])for(var h=0;h<d[0].childNodes.length;h++)switch(d[0].childNodes[h].tagName){
                case "colwidth":
                    d[0].childNodes[h].firstChild&&d[0].childNodes[h].firstChild.data=="%"&&(e="setInitWidthsP");
                    break;
                case "splitat":
                    i=
                    d[0].childNodes[h].firstChild?d[0].childNodes[h].firstChild.data:!1
            }
            this._launchCommands(this.xmlLoader.doXPath("./beforeInit/call",b[0]));
            if(c.length>0){
                this.hdr.rows.length>0&&this.clearAll(!0);
                for(var f=[[],[],[],[],[],[],[],[],[]],k=",width,type,align,sort,color,format,hidden,id".split(","),g=["",e,"setColTypes","setColAlign","setColSorting","setColumnColor","","","setColumnIds"],l=0;l<c.length;l++){
                    for(var j=1;j<k.length;j++)f[j].push(c[l].getAttribute(k[j]));
                    f[0].push((c[l].firstChild?c[l].firstChild.data:
                        "").replace(/^\s*((\s\S)*.+)\s*$/gi,"$1"))
                }
                this.setHeader(f[0]);
                for(l=0;l<g.length;l++)if(g[l])this[g[l]](f[l].join(this.delim));for(l=0;l<c.length;l++)if(this.cellType[l].indexOf("co")==0||this.cellType[l]=="clist"){
                    var m=this.xmlLoader.doXPath("./option",c[l]);
                    if(m.length){
                        var n=[];
                        if(this.cellType[l]=="clist"){
                            for(j=0;j<m.length;j++)n[n.length]=m[j].firstChild?m[j].firstChild.data:"";
                            this.registerCList(l,n)
                        }else for(var o=this.getCombo(l),j=0;j<m.length;j++)o.put(m[j].getAttribute("value"),m[j].firstChild?
                            m[j].firstChild.data:"")
                    }
                }else f[6][l]&&(this.cellType[l].toLowerCase().indexOf("calendar")!=-1||this.fldSort[l]=="date"?this.setDateFormat(f[6][l]):this.setNumberFormat(f[6][l],l));this.init();
        ...
        if(r.length>0){
            this.UserData.gridglobaluserdata||(this.UserData.gridglobaluserdata=
                new Hashtable);
            for(j=0;j<r.length;j++){
                for(var s="",t=0;t<r[j].childNodes.length;t++)s+=r[j].childNodes[t].nodeValue;
                this.UserData.gridglobaluserdata.put(r[j].getAttribute("name"),s)
            }
        }
    },
    ...
    adjustColumnSize:function(a,b){
        if(!this._hrrar||!this._hrrar[a]){
            ...
            for(var i=this.obj.rows.length,h=1;h<i;h++){
                var f=this.obj.rows[h];
                this.rowsAr[f.idd]&&!(f._childIndexes&&f._childIndexes[a]!=a)&&f.childNodes[a]&&
                (f=_isFF||_isOpera||b?f.childNodes[a].textContent.length*this.fontWidth:f.childNodes[a].scrollWidth,f>c&&(c=f))
            }
            c+=20+(b||0);
    ...
    attachHeader:function(a,b,c){
        ...
        c=c||"_aHead";
        if(this.hdr.rows.length)if(a)this._createHRow([a,
            b],this[c=="_aHead"?"hdr":"ftr"]);
        else{
    ...
    _createHRow:function(a,b){
        if(!b){
            ...
            if(!_isIE)d.width="100%",d.style.paddingRight="20px";
            d.style.marginRight=
            "20px";
            ...
            this.ftr=b=d;
            for(var e=d.insertRow(0),i=this.hdrLabels.length<=1?a[0].length:this.hdrLabels.length,h=0;h<i;h++)e.appendChild(document.createElement("TH")),e.childNodes[h]._cellIndex=h;
        ...
        for(h=0;h<a[0].length;h++)if(a[0][h]=="#cspan"){
            var k=c.cells[c.cells.length-
            1];
            k.colSpan=(k.colSpan||1)+1
        }else if(a[0][h]=="#rspan"&&b.rows.length>1){
            for(var g=b.rows.length-2,l=!1,k=null;!l;){
                for(var k=b.rows[g],j=0;j<k.cells.length;j++)if(k.cells[j]._cellIndex==h){
                    l=j+1;
                    break
                }
                g--
            ...
            k=k.cells[l-1];
            k.rowSpan=(k.rowSpan||1)+1
        }else{
            var m=document.createElement("TD");
            m._cellIndex=m._cellIndexS=h;
            if(this._hrrar&&this._hrrar[h]&&!_isIE)m.style.display="none";
            if(typeof a[0][h]=="object")m.appendChild(a[0][h]);
            else if(m.innerHTML=this.forceDivInHeader?"<div class='hdrcell'>"+(a[0][h]||
                "&nbsp;")+"</div>":a[0][h]||"&nbsp;",(a[0][h]||"").indexOf("#")!=-1)if(d=a[0][h].match(/(^|{)#([^}]+)(}|$)/)){
                var n="_in_header_"+d[2];
                if(this[n])this[n](this.forceDivInHeader?m.firstChild:m,h,a[0][h].split(d[0]))
            }
            if(f)m.style.cssText=f[h];
            c.appendChild(m)
        }
        var o=b;
    ...
    forEachRow:function(a){
        for(var b in this.rowsAr)this.rowsAr[b]&&
            this.rowsAr[b].idd&&a.apply(this,[this.rowsAr[b].idd])
    },
    ...
    _refreshFromXML:function(a){
        ...
        var c=a.doXPath("//rows")[0].getAttribute("parent")||
        0,d={};
        ...
        for(var e=a.doXPath("//row"),i=0;i<e.length;i++){
            var h=e[i],f=h.getAttribute("id");
            d[f]=!1;
            c=h.parentNode.getAttribute("id")||c;
            if(this.rowsAr[f]&&this.rowsAr[f].tagName!="TR")this._h2?this._h2.get[f].buff.data=h:this.rowsBuffer[this.getRowIndex(f)].data=h,this.rowsAr[f]=h;
            else if(this.rowsAr[f])this._process_xml_row(this.rowsAr[f],h,-1),this._postRowProcessing(this.rowsAr[f],
                !0);
                ...
                this._refresh_mode[1]=="top"?(this.rowsBuffer.unshift(k),g=0):this.rowsBuffer.push(k);
                if(this._h2)reset=!0,this._h2.add(f,h.parentNode.getAttribute("id")||h.parentNode.getAttribute("parent")).buff=this.rowsBuffer[this.rowsBuffer.length-1];
                this.rowsAr[f]=h;
                h=this.render_row(g);
                this._insertRowAt(h,g?-1:0)
            }
        }
        if(this._refresh_mode[2])for(f in d)d[f]&&this.rowsAr[f]&&
            this.deleteRow(f);this._refresh_mode=null;
    ...
    _key_events:{
        ...
        k13_0_0:function(){
            ...
            this.callEvent("onEnter",
                [this.row?this.row.idd:null,this.cell?this.cell._cellIndex:null]);
        ...
        k9_0_1:function(){
            ...
            var a=this._getNextCell(null,-1);
            if(a)this.selectCell(a.parentNode,a._cellIndex,this.row!=a.parentNode,!1,!0),this._still_active=
                !0
        },
        k113_0_0:function(){
        ...
        k39_0_0:function(){
            if(!this.editor&&
                this.isTreeGrid())this.expandKids(this.row);else return!1
        },
        k40_0_0:function(){
            ...
            if(this.editor&&this.editor.combo)this.editor.shiftNext();
                ...
                if(this.rowsBuffer[b])a._r_select=null,this.selectCell(b,this.cell._cellIndex,!0),a.pagingOn&&a.showRow(a.getRowId(b));
                else{
                    if(!this.callEvent("onLastRow",[]))return!1;
                    this._key_events.k34_0_0.apply(this,[]);
                    this.pagingOn&&this.rowsCol[b]&&
                    this.selectCell(b,0,!0)
                }
            }
            this._still_active=!0
        },
    ...
    _build_master_row:function(){
        for(var a=document.createElement("DIV"),
            b=["<table><tr>"],c=0;c<this._cCount;c++)b.push("<td></td>");
        b.push("</tr></table>");
        a.innerHTML=b.join("");
    ...
    _process_json_row:function(a,b){
        ...
        this._fillRow(a,this._c_order?this._swapColumns(b.data):b.data);
        return a
    },
    ...
    _process_store_row:function(a,b){
        ...
        a._attrs=b;
        this._fillRow(a,this._c_order?this._swapColumns(c):c)
    },
    _process_xml_row:function(a,b){
        ...
        if(this._ud_enabled)for(var e=this.xmlLoader.doXPath("./userdata",b),i=e.length-1;i>=0;i--){
            for(var h="",f=0;f<e[i].childNodes.length;f++)h+=e[i].childNodes[f].nodeValue;
            this.setUserData(a.idd,e[i].getAttribute("name"),h)
        }
        for(f=0;f<c.length;f++){
            var k=c[this._c_order?this._c_order[f]:f];
            if(k){
                var g=a._childIndexes?a._childIndexes[f]:f,l=k.getAttribute("type");
                if(a.childNodes[g]){
                    if(l)a.childNodes[g]._cellType=l;
                    a.childNodes[g]._attrs=this._xml_attrs(k)
                }
                k.getAttribute("xmlcontent")||(k=k.firstChild?
                    k.firstChild.data:"");
                d.push(k)
            }
        }
    ...
    _fillRow:function(a,b){
        ...
        for(var c=0;c<a.childNodes.length;c++)if(c<b.length||this.defVal[c]){
            var d=a.childNodes[c]._cellIndex,
            e=b[d],i=this.cells4(a.childNodes[c]);
            if(this.defVal[d]&&(e==""||typeof e=="undefined"))e=this.defVal[d];
            i&&i.setValue(e)
        }else a.childNodes[c].innerHTML="&nbsp;",a.childNodes[c]._clearCell=!0;return a
    },
    _postRowProcessing:function(a,b){
        ...
        for(var c=0,d=0;d<a.childNodes.length;d++){
            var e=a.childNodes[d],i=e._cellIndex,h=e._attrs.style||a._attrs.style;
            h&&(e.style.cssText+=
                ";"+h);
            if(e._attrs["class"])e.className=e._attrs["class"];
            if(h=e._attrs.align||this.cellAlign[i])e.align=h;
    ...
    load:function(a,b,c){
        ...
        arguments.length==2&&typeof b!="function"&&
        (c=b,b=null);
        c=c||"xml";
        if(!this.xmlFileUrl)this.xmlFileUrl=a;
        this._data_type=c;
        this.xmlLoader.onloadAction=function(a,e,i,h,f){
    ...
    _process_xml:function(a){
        ...
        if(c.tagName.toLowerCase()==this.xml.top){
            this._parseHead(c);
            var d=a.doXPath(this.xml.row,c),e=parseInt(a.doXPath("//"+this.xml.top)[0].getAttribute("pos")||0),i=parseInt(a.doXPath("//"+this.xml.top)[0].getAttribute("total_count")||0),h=!1;
            i&&(this.rowsBuffer[i-1]||(this.rowsBuffer.length&&(h=!0),this.rowsBuffer[i-1]=null),i<this.rowsBuffer.length&&(this.rowsBuffer.splice(i,this.rowsBuffer.length-i),h=!0));
            if(this.isTreeGrid())return this._process_tree_xml(a);
            for(var f=0;f<d.length;f++)if(!this.rowsBuffer[f+e]){
                var k=d[f].getAttribute("id")||f+e+1;
                this.rowsBuffer[f+e]={
                    idd:k,
                    data:d[f],
                    _parser:this._process_xml_row,
    ...
    _process_jsarray:function(a){
        this._parsing=!0;
        if(a&&a.xmlDoc)eval("dhtmlx.temp="+a.xmlDoc.responseText+
            ";"),a=dhtmlx.temp;
        for(var b=0;b<a.length;b++){
            var c=b+1;
    ...
    _process_csv:function(a){
        ...
        for(var c=
            0;c<a.length;c++)if(a[c]||c!=a.length-1){
            if(this._csvAID){
                var d=c+1;
                this.rowsBuffer.push({
                    idd:d,
    ...
    render_dataset:function(a,b){
        ...
        this.pagingOn?(a=Math.max(a||0,(this.currentPage-1)*this.rowsBufferOutSize),b=Math.min(this.currentPage*this.rowsBufferOutSize,
            this.rowsBuffer.length)):(a=a||0,b=b||this.rowsBuffer.length);
        for(var c=a;c<b;c++){
            ...
            if(!d.parentNode||!d.parentNode.tagName)if(this._insertRowAt(d,c),d._attrs.selected||d._attrs.select)this.selectRow(d,d._attrs.call?!0:!1,!0),d._attrs.selected=d._attrs.select=
                null;
        ...
        if(this._srnd&&!this._fillers)this._fillers=[this._add_filler(b,this.rowsBuffer.length-b)];
        this.setSizes()
    },
    ...
    sortRows:function(a,b,c){
        ...
        if(this.isTreeGrid())this.sortTreeRows(a,b,c);
            ...
            for(var h=0;h<this.rowsBuffer.length;h++)d[this.rowsBuffer[h].idd]=this._get_cell_value(this.rowsBuffer[h],a,i);
    ...
    _sortCore:function(a,b,c,d,e){
        ...
        if(b=="str")e[i](function(a,b){
            return c=="asc"?d[a.idd]>d[b.idd]?1:-1:d[a.idd]<d[b.idd]?1:-1
        });
        else if(b=="int")e[i](function(a,b){
            var e=parseFloat(d[a.idd]),e=isNaN(e)?-99999999999999:e,g=parseFloat(d[b.idd]),g=isNaN(g)?-99999999999999:g;
            return c=="asc"?e-g:g-e
        });
        else if(b=="date")e[i](function(a,b){
            var e=Date.parse(d[a.idd])||Date.parse("01/01/1900"),g=Date.parse(d[b.idd])||Date.parse("01/01/1900");
            return c=="asc"?e-g:g-e
        })
    },
    ...
    _reset_view:function(a){
        if(this.obj.rows[0]){
            ...
            if(_isKHTML)for(var d=b.parentNode.childNodes.length-1;d>=0;d--)b.parentNode.childNodes[d].tagName=="TR"&&b.parentNode.removeChild(b.parentNode.childNodes[d],!0);
            else if(_isIE)for(d=b.childNodes.length-1;d>=0;d--)b.childNodes[d].removeNode(!0);else b.innerHTML="";
            b.appendChild(c);
            this.rowsCol=dhtmlxArray();
            this._sst&&
            this.enableStableSorting(!0);
            this._fillers=this.undefined;
    ...
    deleteRow:function(a,b){
        ...
        if(b){
            ...
            if(this.cellType._dhx_find("tree")!=-1&&!this._realfake)c=this._h2.get[a].parent.id,this._removeTrGrRow(b);
                ...
                for(var e=0;e<this.rowsBuffer.length;e++)if(this.rowsBuffer[e]&&
                    this.rowsBuffer[e].idd==a){
    ...
    _addRow:function(a,b,c){
        ...
        if(this._fillers){
            this.rowsCol._dhx_insertAt(c,
                null);
            this.rowsBuffer._dhx_insertAt(c,d);
            this._fake&&this._fake.rowsCol._dhx_insertAt(c,null);
            this.rowsAr[d.idd]=d;
            for(var i=!1,h=0;h<this._fillers.length;h++){
                var f=this._fillers[h];
                if(f&&f[0]<=c&&f[0]+f[1]>=c)f[1]+=1,f[2].firstChild.style.height=parseInt(f[2].firstChild.style.height)+this._srdh+"px",i=!0,this._fake&&this._fake._fillers[h][1]++;
                f&&f[0]>c&&(f[0]+=1,this._fake&&this._fake._fillers[h][0]++)
            }
            i||this._fillers.push(this._add_filler(c,1,c==0?{
    ...
    getRowById:function(a){
        ...
        if(b){
            if(b.tagName!="TR"){
                for(var c=0;c<this.rowsBuffer.length;c++)if(this.rowsBuffer[c]&&this.rowsBuffer[c].idd==a)return this.render_row(c);if(this._h2)return this.render_row(null,b.idd)
            }
            return b
        }
        return null
    ...
    cells:function(a,b){
        ...
        var d=c._childIndexes?c.childNodes[c._childIndexes[b]]:
        c.childNodes[b];
    ...
    setExternalTabOrder:function(a,
        b){
        var c=this;
        this.tabStart=typeof a=="object"?a:document.getElementById(a);
        this.tabStart.onkeydown=function(a){
            var b=a||window.event;
            if(b.keyCode==9)return b.cancelBubble=!0,c.selectCell(0,0,0,0,1),c.smartTabOrder&&c.cells2(0,0).isDisabled()&&c._key_events.k9_0_0.call(c),this.blur(),!1
        };
...
(function(){
    ...
    function e(a,b){
        ...
        for(var d=1,e=[],j=0;j<b.length;j++)if(typeof b[j]=="object"&&b[j].length)for(var i=0;i<b[j].length;i++)c(j,i,b[j][i]);else c(j,
            0,b[j]);for(j=0;j<e.length;j++)for(i=0;i<e[0].length;i++){
            var n=e[j][i];
            e[j][i]=(n||"").toString()||"&nbsp;";
            if(n&&n.colspan)for(var o=1;o<n.colspan;o++)c(i+o,j,"#cspan");
            if(n&&n.rowspan)for(o=1;o<n.rowspan;o++)c(i,j+o,"#rspan")
        }
        this.setHeader(e[0]);
        for(j=1;j<e.length;j++)this.attachHeader(e[j])
    }
    var i=[{
        name:"label",
        def:"&nbsp;",
...
dhtmlXGridObject.prototype._dp_init=function(a){
    ...
    a._getRowData=function(a){
        ...
        for(var d=this.obj.getRowById(a),e=0;e<this.obj._cCount;e++){
            var i=this.obj._c_order?this.obj._c_order[e]:e,h=this.obj.cells(d.idd,e);
            if(!this._changed||h.wasChanged())this._endnm?
                c[this.obj.getColumnId(e)]=h.getValue():c["c"+i]=h.getValue()
        }
        var f=this.obj.UserData[a];
        if(f)for(var k=0;k<f.keys.length;k++)f.keys[k].indexOf("__")!=0&&(c[f.keys[k]]=f.values[k]);
        if(f=this.obj.UserData.gridglobaluserdata)for(k=0;k<f.keys.length;k++)c[f.keys[k]]=f.values[k];
        return c
    };
    ...
    a.checkBeforeUpdate=function(a){
        for(var c=!0,d=[],e=0;e<this.obj._cCount;e++)if(this.mandatoryFields[e]){
            var i=
            this.mandatoryFields[e].call(this.obj,this.obj.cells(a,e).getValue(),a,e);
